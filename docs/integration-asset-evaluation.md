...
حزمة `v86@0.5.441` الموجودة أصلاً في بيئة البناء تبلغ نحو **18 MiB**، وينتج منها artifact WebAssembly وظيفي بحجم نحو **2.1 MiB** ضمن build. هذا هو المورد الكبير الوحيد الذي يصل إلى المستخدم الآن، ويعمل كموصل اختياري لبيئة FreeDOS القانونية. لا فائدة من نسخ الحزمة أو تخزين artifacts المولدة في Git؛ يعيد `pnpm install` و`pnpm build` إنتاجها من dependency المقفلة في `pnpm-lock.yaml`.

لم يُضف SeaBIOS أو Bochs أو KVM-unit-tests أو أي صورة ضيف في هذه المرحلة. SeaBIOS وBochs يحملان التزامات copyleft مختلفة، وKVM-unit-tests مرجع GPL لا يُنسخ إلى مشروعنا. المسار العملي التالي للأصول الكبيرة هو corpus اختبارات مستقل برخصة واضحة أو صورة ضيف مفتوحة المصدر مع SHA-256 في object storage، بعد أن نضيف adapter الاختبار الذي يستهلكها بالفعل.

...
| الذاكرة | PML4 رباعي المستويات وصفحات 4KiB وعناوين canonical، مع backing sparse حتى 4GiB يخصص صفحات 4KiB الملموسة فقط | اختبارات read/write ورفض page fault وترجمة صفحة فوق 2GiB |
...
| وسيط محلي | `Blob/File` يقرأ قطاعات 512 بايت عند الطلب ضمن cache محدود؛ مسار v86 يقبل `File` متدرجاً بدلاً من نسخ ISO كاملاً إلى `ArrayBuffer` | اختبارات عدم التعريض قبل cache وترتيب storage→PIC وتجربة ReactOS محلية |
| replay | مقارنة trace مع snapshot معماري للسجلات والذاكرة المراقبة وRIP/RSP/RFLAGS؛ fixture Core-64 متكرر يبين انحراف الحالة الأول | اختبارات repeat/divergence وPS/2 وdevice replay |
...
لا ينفذ Core-64 حتى الآن سوى جزء محدد من address forms في ModR/M ولا ينفذ انتقال امتياز كاملًا أو TSS hardware stack switching أو إطارات SS/RSP0. كما أن PIC/CMOS/ATA نماذج منصة محدودة بعقود منافذ واختبارات، وليست مسار إقلاع قرص أو توافق firmware كامل. لا توجد large pages أو NX أو USB guest controller أو نموذج IDT faults كامل أو مجموعة تعليمات كافية لنواة حديثة. الذاكرة sparse داخل Core-64 ليست حجزاً فعلياً لـ4GiB من RAM المتصفح؛ واجهة v86 تعرض الآن حتى 4GiB كطلب محلي مباشر للمستخدم على كل المنصات. وصل اختبار ReactOS LiveCD المحلي إلى SeaBIOS وFreeLoader فقط ولم يثبت سطح مكتب ReactOS. لذلك لا يشغل Windows أو Linux أو ReactOS كاملاً حالياً، ولا يجب تقديمه على أنه يفعل ذلك. يظل الوسيط محلياً يختاره المستخدم ولا يُرفع أو يُضمَّن في المستودع.

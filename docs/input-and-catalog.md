...
## USB وHID: تكامل اختياري لا بديل للإدخال الأساسي

تجعل JustGo مسار **PS/2 المحاكى مع Pointer Events** قاعدة الإدخال التي تعمل بلا طلب صلاحية أجهزة. عند توافر WebHID أو WebUSB، تظهر للمستخدم عملية اختيار جهاز صريحة تبدأها نقرة منه فقط، وتبقى الميزة اختيارية ولا تمنع الإقلاع عند غيابها. توثق MDN أن WebHID وWebUSB واجهات secure-context وغير متاحة في Firefox، كما أن دعمهما يتركز في متصفحات Chromium؛ لذلك لا تدّعي JustGo دعماً متساوياً في Safari أو iOS.[7] [8]

| المسار | الغرض في JustGo | شرط الأمان | بديل عند الغياب |
|---|---|---|---|
| Pointer Events + PS/2 | ماوس ولوحة مفاتيح افتراضيان | أحداث الصفحة العادية فقط | لمس مباشر وإيماءات ظاهرة |
| WebHID | أجهزة HID منتقاة بعد موافقة المستخدم | HTTPS + اختيار جهاز من المستخدم | PS/2 المحاكى |
| WebUSB | أجهزة USB غير القياسية عند دعم المتصفح | HTTPS + اختيار جهاز من المستخدم | لا وصول خام تلقائي؛ يبقى الجهاز خارج الضيف |

لن تنشئ JustGo اتصال USB تلقائياً، ولن تطلب أذناً عند تحميل الصفحة، ولن تخلط وصول WebUSB بالقدرة على محاكاة USB كامل داخل الضيف. يحتاج USB الافتراضي نفسه إلى متحكم ضيف ووصف أجهزة ونقل endpoints، وهي طبقة مستقلة تضاف تدريجياً وتختبر منفصلة.
...
[7] [WebHID API — MDN](https://developer.mozilla.org/en-US/docs/Web/API/WebHID_API)

[8] [WebUSB API — MDN](https://developer.mozilla.org/en-US/docs/Web/API/WebUSB_API)
